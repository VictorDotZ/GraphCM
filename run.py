...
def parse_args():
    ...
    parser.add_argument(
        "--test", action="store_true", help="perform click prediction task on test set"
    )
    parser.add_argument(
        "--rank",
        action="store_true",
    ...
    train_settings.add_argument("--optim", default="adadelta", help="optimizer type")
    ...
    train_settings.add_argument("--momentum", type=float, default=0.99, help="momentum")
    train_settings.add_argument(
    ...
    model_settings.add_argument(
        "--algo", default="GraphCM", help="choose the algorithm to use"
    )
    model_settings.add_argument(
        "--combine",
        default="mul",
    ...
    model_settings.add_argument(
        "--embed_size", type=int, default=64, help="size of the query/doc embeddings"
    )
    model_settings.add_argument(
        "--pos_embed_size", type=int, default=4, help="size of the position embeddings"
    )
    model_settings.add_argument(
        "--click_embed_size", type=int, default=4, help="size of the click embeddings"
    )
    model_settings.add_argument(
        "--vtype_embed_size", type=int, default=8, help="size of the vtype embeddings"
    )
    model_settings.add_argument(
        "--hidden_size", type=int, default=128, help="size of RNN hidden units"
    )
    model_settings.add_argument(
        "--max_d_num", type=int, default=10, help="max number of docs in a session"
    )
    model_settings.add_argument(
        "--use_pretrain_embed",
        action="store_true",
    ...
    model_settings.add_argument(
        "--use_gnn", action="store_true", help="whether use gnn layer"
    )
    model_settings.add_argument(
        "--gnn_neigh_sample",
        type=int,
    ...
    path_settings.add_argument(
        "--dataset", default="TianGong-ST", help="name of the dataset to be used"
    )
    path_settings.add_argument(
        "--model_dir", default="./outputs/models/", help="the dir to store models"
    )
    path_settings.add_argument(
        "--result_dir",
    ...
    path_settings.add_argument(
        "--check_point", type=int, default=100, help="the frequency of saving model"
    )
    path_settings.add_argument(
        "--patience",
        type=int,
    ...
    path_settings.add_argument("--lr_decay", type=float, default=0.5, help="lr decay")
    path_settings.add_argument(
        "--load_model", type=int, default=-1, help="load model global step"
    )
    path_settings.add_argument(
        "--data_parallel", type=bool, default=False, help="data_parallel"
    )
    path_settings.add_argument("--gpu_num", type=int, default=1, help="gpu_num")
...
def valid(args, dataset):
    ...
    assert (
        args.load_model > -1
    ), "args.load_model is required to specify the model file to be loaded!"
    logger.info("Reloading the model...")
    model.load_model(
        model_dir=args.model_dir, model_prefix=args.algo, global_step=args.load_model
    )
...
def rank(args, dataset):
    ...
    for i in range(args.num_iter):
        ...
        for trunc_level in trunc_levels:
            ...
            summary_writer.add_scalar(
                "final_NDCG/avg_{}".format(trunc_level),
                sum_ndcgs[trunc_level] / (i + 1),
                i,
            )
            summary_writer.add_scalar(
