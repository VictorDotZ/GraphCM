...
def generate_list_dict(args):
    step_pbar = tqdm(total=425951)  # There are totally 452,117 sessions
    session_sid, query_qid, url_uid, vtype_vid = (
        {"": 0},
        {"": 0},
        {"": 0},
        {"": 0, "1": 1},
    ...
    for line in open(os.path.join(args.input, "iphone-20240201")):
        ...
        if len(infos_per_session) > 0:
            ...
            assert len(infos_per_session[-1]["qids"]) == len(
                infos_per_session[-1]["uidsS"]
            )
            assert len(infos_per_session[-1]["qids"]) == len(
                infos_per_session[-1]["clicksS"]
            )
        ...
        for click in elements[2].split(","):
            clicked_uid = url_uid[serp[int(click)]]
    ...
    assert len(infos_per_session) == 425951

    # Downsample the dataset due to memory limitation
    print(
        "  - {}".format(
            "Downsampling the dataset from {} sessions to {} sessions".format(
                len(infos_per_session), args.downsample
            )
        )
    )
...
def generate_train_valid_test(args):
    ...
    assert train_session_num == len(
        ...
        train_session_num, len(sessions["train"])
    )
    assert valid_session_num == len(
        sessions["valid"]
    ), "valid_session_num: {}, len(valid_sessions): {}".format(
        valid_session_num, len(sessions["valid"])
    )
    assert test_session_num == len(
        sessions["test"]
    ), "test_session_num: {}, len(test_sessions): {}".format(
        test_session_num, len(sessions["test"])
    )
    assert session_num == len(sessions["train"]) + len(sessions["valid"]) + len(
        sessions["test"]
    ), "session_num: {}, len(train_sessions) + len(valid_sessions) + len(test_sessions): {}".format(
        session_num,
        len(sessions["train"]) + len(sessions["valid"]) + len(sessions["test"]),
    )
    ...
    for file_type in file_types:
        ...
        file = open(
            os.path.join(args.output, "{}_per_query_quid.txt".format(file_type)), "w"
        )
        for info_per_session in sessions[file_type]:
            ...
            for qid, uids, clicks in zip(qids, uidsS, clicksS):
                ...
                file.write(
                    "{}\t{}\t{}\t{}\t{}\n".format(
                        sid, qid_print, str(uids_print), str([1] * 10), str(clicks)
                    )
                )
...
def construct_dgat_graph(args):
    ...
    for set_name in set_names:
        ...
        lines = open(
            os.path.join(args.output, "{}_per_query_quid.txt".format(set_name))
        ).readlines()
    ...
    # print(qid_edges)
    # print(uid_edges)
    qid_edge_index = torch.transpose(
        torch.from_numpy(np.array(qid_edges, dtype=np.int64)), 0, 1
    )
    uid_edge_index = torch.transpose(
        torch.from_numpy(np.array(uid_edges, dtype=np.int64)), 0, 1
    )
    ...
    non_isolated_qid_cnt = sum(
        [1 if qid_degree > 1 else 0 for qid_degree in qid_degrees]
    )
    non_isolated_uid_cnt = sum(
        [1 if uid_degree > 1 else 0 for uid_degree in uid_degrees]
    )
    print(
        "  - {}".format(
            "Mean/Max/Min qid degree: {}, {}, {}".format(
                sum(qid_degrees) / len(qid_degrees), max(qid_degrees), min(qid_degrees)
            )
        )
    )
    print(
        "  - {}".format(
            "Mean/Max/Min uid degree: {}, {}, {}".format(
                sum(uid_degrees) / len(uid_degrees), max(uid_degrees), min(uid_degrees)
            )
        )
    )
    ...
    for idx, adj in enumerate(uid_neigh):
        ...
        if len(adj_list) >= max_node_degree:
            adj_sample = torch.from_numpy(
                np.array(random.sample(adj_list, max_node_degree), dtype=np.int64)
            )
        else:
            adj_sample = torch.from_numpy(
                np.array(random.choices(adj_list, k=max_node_degree), dtype=np.int64)
            )
...
def generate_dataset_for_cold_start(args):
    ...
    print(
        "    - {}".format(
            "Total session num: {}".format(
                len(cold_q) + len(cold_d) + len(cold_qd) + len(warm_qd)
            )
        )
    )
    ...
    for session_info in cold_q:
        ...
        for qid, uids, vids, clicks in zip(qids, uidsS, vidsS, clicksS):
            file.write(
                "{}\t{}\t{}\t{}\t{}\n".format(
                    sid, qid, str(uids), str(vids), str(clicks)
                )
            )
    file.close()
...
def compute_sparsity(args):
    ...
    for set_name in set_names:
        print(
            "    - {}".format(
                "Counting the query-doc pairs in the {} set".format(set_name)
            )
        )
        lines = open(
            os.path.join(args.output, "{}_per_query_quid.txt".format(set_name))
    ...
    print(
        "  - {}".format(
            "There are {} unique query-doc pairs in the training dataset...".format(
                len(train_qu_set)
            )
        )
    )
    print(
        "  - {}".format(
            "There are {} unique queries in the dataset...".format(len(q_set))
        )
    )
    print(
        "  - {}".format("There are {} unique docs in the dataset...".format(len(u_set)))
    )
    print(
        "  - {}".format(
            "There are {} possible query-doc pairs in the whole dataset...".format(
                len(q_set) * len(u_set)
            )
        )
    )
    print(
        "  - {}".format(
            "The sparsity is: 1 - {} / {} = {}%".format(
                len(train_qu_set),
                len(q_set) * len(u_set),
                100 - 100 * len(train_qu_set) / (len(q_set) * len(u_set)),
            )
        )
    )
...
def main():
    parser = argparse.ArgumentParser("VK")
    parser.add_argument("--input", default="./dataset/VK/", help="input path")
    parser.add_argument("--output", default="./data/VK", help="output path")
    parser.add_argument(
        "--list_dict", action="store_true", help="generate list & dict files"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dgat", action="store_true", help="construct graph for double GAT"
    )
    parser.add_argument(
        "--cold_start",
        action="store_true",
    ...
    parser.add_argument(
        "--sparsity", action="store_true", help="compute sparisity for the dataset"
    )
    parser.add_argument(
        "--trainset_ratio",
        default=0.8,
    ...
    if args.cold_start:
        ...
        print(
            "===> {}".format("generating dataset for studying cold start problems...")
        )
