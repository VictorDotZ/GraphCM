...
class Dataset(object):
    ...
    def __init__(self, args):
        ...
        self.train_set = self.load_dataset(
            os.path.join(self.data_dir, "train_per_query_quid.txt"), mode="train"
        )
        self.valid_set = self.load_dataset(
            os.path.join(self.data_dir, "valid_per_query_quid.txt"), mode="valid"
        )
        self.test_set = self.load_dataset(
            os.path.join(self.data_dir, "test_per_query_quid.txt"), mode="test"
        )
        ...
        self.logger.info(
            "Unique query num, including zero vector: {}".format(self.query_size)
        )
        self.logger.info(
            "Unique doc num, including zero vector: {}".format(self.doc_size)
        )
        self.logger.info(
            "Unique vtype num, including zero vector: {}".format(self.vtype_size)
        )
    ...
    def load_dataset(self, data_path, mode):
        ...
        for line in lines:
            ...
            if previous_sid != sid:
                    ...
                    data_set.append(
                        {
                            ...
                            "relevances": relevances[
                                relevance_start : relevance_start + 10
                            ],
                ...
                relevances = (
                    json.loads(attr[5].strip())
                    if mode == "label"
                    else [0 for _ in range(self.max_d_num)]
                )
            else:
