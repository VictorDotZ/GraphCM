...
class Model(object):
    ...
    def compute_perplexity(self, pred_logits, TRUE_CLICKS, MASK):
        ...
        neg_logits = torch.log2(1.0 - pred_logits + MINF)
        perplexity_at_rank = torch.where(TRUE_CLICKS == 1, pos_logits, neg_logits)
        perplexity_at_rank = torch.where(
            MASK == True,
            perplexity_at_rank,
            torch.zeros(perplexity_at_rank.shape, device=device),
        )
        for session_idx in range(1, session_num):
            perplexity_at_rank[:, :10] += perplexity_at_rank[
                :, 10 * session_idx : 10 * session_idx + 10
            ]
    ...
    def _train_epoch(self, train_batches, dataset, metric_save, patience, step_pbar):
        ...
        for b_idx, batch in enumerate(train_batches):
            ...
            TRUE_CLICKS = rnn_utils.pad_sequence(
                [
                    torch.from_numpy(np.array(true_click, dtype=np.float32))
                    for true_click in batch["true_clicks"]
                ],
                batch_first=True,
            )
            MASK = rnn_utils.pad_sequence(
                [torch.ones(len(true_click)) for true_click in batch["true_clicks"]],
                batch_first=True,
            )
            MASK = MASK == 1
            ...
            pred_logits, pred_rels = self.model(
                batch["qids"], batch["uids"], batch["vids"], batch["clicks"]
            )
            ...
            if evaluate and self.global_step % self.eval_freq == 0:
                ...
                valid_click_loss, valid_rel_loss, valid_perplexity = self.evaluate(
                    valid_batches, dataset
                )
                torch.cuda.empty_cache()
                self.writer.add_scalar(
                    "valid/click_loss", valid_click_loss, self.global_step
                )
                self.writer.add_scalar(
                    "valid/perplexity", valid_perplexity, self.global_step
                )

                test_batches = dataset.gen_mini_batches(
                    "test", dataset.testset_size, shuffle=False
                )
                test_click_loss, test_rel_loss, test_perplexity = self.evaluate(
                    test_batches, dataset
                )
                torch.cuda.empty_cache()
                self.writer.add_scalar(
                    "test/click_loss", test_click_loss, self.global_step
                )
                self.writer.add_scalar(
                    "test/perplexity", test_perplexity, self.global_step
                )
                ...
                if patience >= self.patience:
                    ...
                    self.writer.add_scalar(
                        "train/lr", self.learning_rate, self.global_step
                    )
                    metric_save = valid_perplexity
                    patience = 0
                    self.patience += 1
    ...
    def train(self, dataset):
        ...
        while not exit_tag:
            train_batches = dataset.gen_mini_batches(
                "train", self.args.batch_size, shuffle=True
            )
            exit_tag, metric_save, patience = self._train_epoch(
                train_batches, dataset, metric_save, patience, step_pbar
            )

    def evaluate(self, eval_batches, dataset):
        total_click_loss, total_rel_loss, total_num = 0.0, 0.0, 0
        perplexity_at_rank = torch.zeros(
            10, device=device, dtype=torch.float
        )  # 10 docs per query
        with torch.no_grad():
            for b_idx, batch in enumerate(eval_batches):
                ...
                batch_perplexity_at_rank = self.compute_perplexity(
                    pred_logits, TRUE_CLICKS, MASK
                )
        ...
        with torch.no_grad():
            for b_idx, batch in enumerate(label_batches):
                ...
                for r_idx, relevance_start in enumerate(batch["relevance_starts"]):
                    relevances_batches[r_idx] = pred_logits[
                        r_idx, relevance_start : relevance_start + 10
                    ]
                relevances_batches = relevances_batches.data.cpu().numpy().tolist()

                for relevances, true_relevances in zip(
                    relevances_batches, true_relevances_batches
                ):
                    pred_rels = {}
                    ...
                    for k in self.trunc_levels:
                        ideal_ranking_relevances = sorted(
                            true_relevances, reverse=True
                        )[:k]
                        ranking = sorted(
                            [idx for idx in pred_rels],
                            key=lambda idx: pred_rels[idx],
                            reverse=True,
                        )
                        ranking_relevances = [
                            true_relevances[idx] for idx in ranking[:k]
                        ]
    ...
    def dcg(self, ranking_relevances):
        ...
        return sum(
            [
                (2**relevance - 1) / math.log(rank + 2, 2)
                for rank, relevance in enumerate(ranking_relevances)
            ]
        )
    ...
    def save_model(self, model_dir, model_prefix):
        ...
        torch.save(
            ...
            os.path.join(
                model_dir, model_prefix + "_{}.model".format(self.global_step)
            ),
        )
        torch.save(
            self.optimizer.state_dict(),
            os.path.join(
                model_dir, model_prefix + "_{}.optimizer".format(self.global_step)
            ),
        )
        self.logger.info(
            "Model and optimizer saved in {}, with prefix {} and global step {}.".format(
                model_dir, model_prefix, self.global_step
            )
        )
    ...
    def load_model(self, model_dir, model_prefix, global_step):
        ...
        optimizer_path = os.path.join(
            model_dir, model_prefix + "_{}.optimizer".format(global_step)
        )
        self.optimizer.load_state_dict(torch.load(optimizer_path))
        self.logger.info(
            "Optimizer restored from {}, with prefix {} and global step {}.".format(
                model_dir, model_prefix, global_step
            )
        )
        model_path = os.path.join(
            model_dir, model_prefix + "_{}.model".format(global_step)
        )
        if use_cuda:
        ...
        self.logger.info(
            "Model restored from {}, with prefix {} and global step {}.".format(
                model_dir, model_prefix, global_step
            )
        )
